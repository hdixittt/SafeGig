...
export default function Policies() {
  ...
  useEffect(() => {
    api.get('/admin/policies').then(r => setPolicies(r.data)).catch(() => {
      setPolicies([
        { id:'p1', worker_name:'Mayank Sharma', city:'Gurgaon', premium_paid:84, coverage_amount:3000, risk_score:0.73, status:'active', week_start:'2026-04-03', week_end:'2026-04-10' },
        { id:'p2', worker_name:'Vartika Singh',  city:'Chennai', premium_paid:104, coverage_amount:4000, risk_score:0.81, status:'active', week_start:'2026-04-03', week_end:'2026-04-10' },
        { id:'p3', worker_name:'Rahul Verma',    city:'Delhi',   premium_paid:49,  coverage_amount:1500, risk_score:0.45, status:'expired', week_start:'2026-03-27', week_end:'2026-04-03' },
      ]);
    });
  }, []);
              ...
              <motion.tr key={p.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.04}}
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 w-16 rounded-full bg-white/10 overflow-hidden"><div className="h-full rounded-full bg-orange-400" style={{width:`${p.risk_score*100}%`}} /></div>
                    <span className="text-xs font-bold text-orange-400">{(p.risk_score*100).toFixed(0)}%</span>
                  </div>
                </td>
